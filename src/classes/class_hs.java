	...
	public final void run() {
		try {
			while (this.a.b()) {
				...
				final Message class_abs = new Message(b, array2);
				try {
					if (class_abs.COMMAND == -40) {
						final Message class_abs2 = class_abs;
						final byte byte1 = class_abs2.reader().readByte();
						this.a.h = new byte[byte1];
						for (byte b2 = 0; b2 < byte1; ++b2) {
							this.a.h[b2] = class_abs2.reader().readByte();
						}
						...
						MessageHandlerClone.onMessage(this.a.b, class_abs);
					}
				} catch (Exception ex) {
					ex.printStackTrace();
				}
		...
		if (this.a.c) {
			if (this.a.b != null) {
				if (System.currentTimeMillis() - this.a.i > 500L) {
					final MessageHandler b3 = this.a.b;
					class_acv.s.n();

				} else {
					final MessageHandler b4 = this.a.b;
					class_acv.s.m();
